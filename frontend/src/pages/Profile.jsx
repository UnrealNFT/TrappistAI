...
export default function Profile({ wallet }) {
  // Tab state
  const [activeTab, setActiveTab] = useState('telegram') // telegram, gallery, rwa
  ...
  // Separate gallery items (not tokenized) from RWA items (tokenized on-chain)
  const galleryItems = tokens.filter(t => !t.cspr_tx_hash)
  const rwaItems = tokens.filter(t => t.cspr_tx_hash)
