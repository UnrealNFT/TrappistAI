...
async def on_free_message(update: Update, context) -> None:
    ...
    if PRICES_AVAILABLE:
        try:
            # Priority: a pasted contract address → full DexScreener token lookup
            if find_contract_address(prompt):
                price_context = await asyncio.get_event_loop().run_in_executor(
                    None, contract_context, prompt
                )
                if price_context:
                    logger.info("🔗 Injected DexScreener contract lookup")
